...
PLUGINLIB_EXPORT_CLASS(p12_llc_local_planner::LLCLocalPlanner, nav_core::BaseLocalPlanner)

using namespace std;

namespace p12_llc_local_planner{
	...
	bool LLCLocalPlanner::computeVelocityCommands(geometry_msgs::Twist& cmd_vel){
		...
		cmd_vel.linear.x = (cmd_vel.linear.x >= 1.0) ? 0.95 : cmd_vel.linear.x;
	...
	bool LLCLocalPlanner::isGoalReached(){
		...
		if(euclideanDistance(robot_pose.pose, goal.pose) < 0.2)
			goalReached = true;
